...
type CostSortField = "net_margin_pct" | "food_cost_pct" | "labor_cost_pct" | "utility_cost_pct";
type SortDirection = "asc" | "desc";
type CostSortConfig = { field: CostSortField; direction: SortDirection };

function getCostPercentage(amount: number, sellingPrice: number) {
  return sellingPrice > 0 ? (amount / sellingPrice) * 100 : null;
}

function getCostSortValue(row: CostAnalysisOut, field: CostSortField) {
  if (row.cost_status && row.cost_status !== "ok") return null;
  if (field === "net_margin_pct") return row.net_margin_pct;
  if (field === "food_cost_pct") return getCostPercentage(row.food_cost, row.selling_price);
  if (field === "labor_cost_pct") return getCostPercentage(row.labor_cost, row.selling_price);
  return getCostPercentage(row.utility_cost, row.selling_price);
}

function CostSortHeader({
  field,
  label,
  sortConfig,
  onSort,
  ...
  field: CostSortField;
  label: string;
  sortConfig: CostSortConfig | null;
  onSort: (field: CostSortField) => void;
}) {
  ...
  return (
    ...
    >
      <button
        ...
        className="inline-flex min-h-10 w-full cursor-pointer items-center justify-end gap-1.5 rounded-lg px-1 text-right transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30"
...
function CostMetric({ amount, percentage }: { amount: number; percentage: number | null }) {
  return (
    <div className="whitespace-nowrap font-mono tabular-nums">
      ...
      <span className="mt-0.5 block text-[10px] font-bold text-slate-400">
        {percentage === null ? "No retail SRP" : `${percentage.toFixed(2)}% of SRP`}
      </span>
    </div>
  );
}

function CostingLedgerRow({
  row,
  showCategory,
  showFinancials,
  onViewBom,
  ...
  onViewBom: (sku: string) => void;
}) {
  const hasValidCost = !row.cost_status || row.cost_status === "ok";
  const foodPercentage = getCostPercentage(row.food_cost, row.selling_price);
  const laborPercentage = getCostPercentage(row.labor_cost, row.selling_price);
  const utilityPercentage = getCostPercentage(row.utility_cost, row.selling_price);

  return (
    <TableRow>
      <TableCell className="min-w-[280px] border-r border-slate-200">
        <ProductDisplay
          sku={row.sku}
      ...
      {showFinancials && (
        <>
          ...
          <TableCell align="right" className="border-r border-slate-200">
            {hasValidCost ? (
              <>
                <CostMetric amount={row.food_cost} percentage={foodPercentage} />
                {row.cost_override !== null && row.cost_override > 0 && (
                  <Badge variant="warning" className="mt-1 rounded px-1.5 py-0.5 text-[10px] font-bold">Override Rule Active</Badge>
                )}
              </>
            ) : (
          ...
          <TableCell align="right" className="border-r border-slate-200">
            <CostMetric amount={row.labor_cost} percentage={laborPercentage} />
          </TableCell>
          <TableCell align="right" className="border-r border-slate-200">
            <CostMetric amount={row.utility_cost} percentage={utilityPercentage} />
          </TableCell>
          <TableCell align="right" className="border-r border-slate-200 font-mono font-black tabular-nums text-emerald-600">
            {hasValidCost ? formatCurrency(row.net_profit) : "Unavailable"}
          </TableCell>
          <TableCell align="right" className="border-r border-slate-200">
            {hasValidCost ? (
              <Badge variant={row.net_margin_pct > 50 ? "success" : (row.net_margin_pct < 40 ? "danger" : "neutral")} className="rounded-lg px-2.5 py-1 text-xs font-bold 2xl:text-sm">
                {row.net_margin_pct}%
              </Badge>
            ) : (
...
function CostingLedgerCard({
  ...
  const hasValidCost = !row.cost_status || row.cost_status === "ok";
  const metrics = [
    { label: "Retail SRP", value: formatCurrency(row.selling_price) },
    {
      label: "Food cost",
      value: hasValidCost
        ? `${formatCurrency(row.food_cost)} · ${getCostPercentage(row.food_cost, row.selling_price)?.toFixed(2) ?? "—"}%`
        : "Needs review",
    },
    {
      label: "Labor",
      value: `${formatCurrency(row.labor_cost)} · ${getCostPercentage(row.labor_cost, row.selling_price)?.toFixed(2) ?? "—"}%`,
    },
    {
      label: "Utility",
      value: `${formatCurrency(row.utility_cost)} · ${getCostPercentage(row.utility_cost, row.selling_price)?.toFixed(2) ?? "—"}%`,
    },
    { label: "Net profit", value: hasValidCost ? formatCurrency(row.net_profit) : "Unavailable" },
    { label: "Profit margin", value: hasValidCost ? `${row.net_margin_pct}%` : "Unavailable" },
  ];
  ...
  return (
    <article className="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {!hasValidCost && (
        <StatusBadge
          status="failed"
          label={row.cost_status_message || "Review costing data"}
          className="mt-3 max-w-full whitespace-normal text-left text-[11px]"
        />
      )}
      ...
      {showFinancials && (
        <dl className="mt-4 grid grid-cols-2 gap-2">
          {metrics.map((metric) => (
            <div key={metric.label} className="min-w-0 rounded-xl bg-[#fbf8f2] p-3">
              <dt className="text-[10px] font-black uppercase tracking-wide text-slate-500">{metric.label}</dt>
              <dd className="mt-1 break-words font-mono text-xs font-black tabular-nums text-slate-900">
                {metric.value}
              </dd>
            </div>
          ))}
...
export default function RecipesPage() {
  ...
  const [costSort, setCostSort] = useState<CostSortConfig | null>(null);
  ...
  const [liveDraftCost, setLiveDraftCost] = useState<{ batchCost: number; portionCost: number } | null>(null);
  const [liveDraftCostLoading, setLiveDraftCostLoading] = useState(false);
  ...
  const fetchData = async (isBackground = false) => {
    ...
    try {
      ...
      const filteredProds = prods.filter((p: any) => p.sku !== "SKU" && p.is_active !== false && isCurrentLineupProduct(p));
    ...
    const request = window.setTimeout(async () => {
      try {
        ...
        if (cachedAnalysis && cachedProducts && cachedRaws && cachedGiftSets && cachedOverheadRates) {
          ...
          setProducts(JSON.parse(cachedProducts).filter((p: any) => p.sku !== "SKU" && p.is_active !== false && isCurrentLineupProduct(p)));
  ...
  const handleOpenBulkEditor = () => {
    ...
    setLiveDraftCost({
      batchCost: Number(details.calculated_batch_cost) || 0,
      portionCost: Number(details.calculated_portion_cost) || 0,
    });
    setIsEditing(true);
  };

  useEffect(() => {
    if (!selectedSku || !isEditing) return;

    const validHeader = editYieldWeight > 0 && editPortionSize > 0
      && editYieldUnit.trim().length > 0 && editPortionUnit.trim().length > 0;
    const validIngredients = editIngredients.every((item) => (
      Number.isFinite(Number(item.base_qty)) && Number(item.base_qty) >= 0
      && String(item.base_unit || "").trim().length > 0
      && (
        (item.ingredient_type === "raw" && Number(item.raw_ingredient_id) > 0)
        || (item.ingredient_type === "sku" && String(item.sub_sku || "").length > 0)
      )
    ));
    if (!validHeader || !validIngredients) {
      const resetTimer = window.setTimeout(() => {
        setLiveDraftCost(null);
        setLiveDraftCostLoading(false);
      }, 0);
    ...
    const timer = window.setTimeout(async () => {
      setLiveDraftCostLoading(true);
      try {
        const preview = await api.previewSkuCost(selectedSku, {
          ...
          notes: editNotes,
          ingredients: editIngredients.map((item) => ({
            ingredient_type: item.ingredient_type,
            raw_ingredient_id: item.ingredient_type === "raw" ? Number(item.raw_ingredient_id) : null,
            sub_sku: item.ingredient_type === "sku" ? item.sub_sku : null,
        ...
        if (!cancelled) {
          setLiveDraftCost({
            batchCost: preview.calculated_batch_cost,
            portionCost: preview.calculated_portion_cost,
          });
        }
      } catch (error) {
        if (!cancelled) {
          console.error("Failed to preview normalized recipe cost:", error);
          setLiveDraftCost(null);
        }
      } finally {
        if (!cancelled) setLiveDraftCostLoading(false);
      }
    }, 250);
  ...
  const sortedAnalysis = costSort
    ? [...filteredAnalysis].sort((left, right) => {
        const leftValue = getCostSortValue(left, costSort.field);
        const rightValue = getCostSortValue(right, costSort.field);
        ...
        return difference === 0
          ...
          : costSort.direction === "asc" ? difference : -difference;
      })
    : filteredAnalysis;
  ...
  const selectedCostRow = selectedSku ? analysis.find((row) => row.sku === selectedSku) : undefined;
  const selectedProduct = selectedSku ? products.find((product) => product.sku === selectedSku) : undefined;
  const selectedSellingPrice = selectedCostRow?.selling_price ?? selectedProduct?.retail_price ?? 0;
  const selectedFoodCost = selectedCostRow?.food_cost ?? details?.calculated_portion_cost ?? selectedProduct?.cost_per_unit ?? 0;
  const selectedLaborCost = selectedCostRow?.labor_cost ?? selectedProduct?.labor_cost ?? 0;
  const selectedUtilityCost = selectedCostRow?.utility_cost ?? selectedProduct?.utility_cost ?? 0;
  const selectedTotalCost = selectedCostRow?.total_cost ?? (selectedFoodCost + selectedLaborCost + selectedUtilityCost);
  const selectedNetProfit = selectedCostRow?.net_profit ?? (selectedSellingPrice - selectedTotalCost);
  const selectedMarginPct = selectedCostRow?.net_margin_pct ?? (
    selectedSellingPrice > 0 ? (selectedNetProfit / selectedSellingPrice) * 100 : 0
  );
  ...
  return (
    <div className="space-y-5 2xl:space-y-6 flex flex-col pb-16">
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:hidden">
              {sortedAnalysis.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center md:col-span-2">
                  <p className="text-sm font-black text-slate-700">No products in this view</p>
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    Choose another category or recalculate costs to refresh the ledger.
                  </p>
                </div>
                ...
                <CostingLedgerCard
                  key={row.sku}
                  row={row}
                  showFinancials={userRole === "owner"}
                  onViewBom={fetchDetails}
                />
              ))}
            </div>

            <DataTableShell className="hidden rounded-3xl xl:block">
                <DataTableScroll label="Product costing ledger" className="overflow-x-auto">
                  <table className="w-full min-w-[1120px] border-collapse text-left text-sm text-slate-700">
                    <thead>
                      <TableHeaderRow>
                        <TableHeaderCell className="min-w-[280px] border-r border-slate-200">Product Name &amp; SKU</TableHeaderCell>
                        {userRole === "owner" && (
                          <>
                            <TableHeaderCell align="right" className="border-r border-slate-200">Retail SRP</TableHeaderCell>
                            <CostSortHeader field="food_cost_pct" label="Food Cost %" sortConfig={costSort} onSort={handleCostSort} />
                            <CostSortHeader field="labor_cost_pct" label="Labor Cost %" sortConfig={costSort} onSort={handleCostSort} />
                            <CostSortHeader field="utility_cost_pct" label="Utility Cost %" sortConfig={costSort} onSort={handleCostSort} />
                            <TableHeaderCell align="right" className="border-r border-slate-200">Net Profit</TableHeaderCell>
                            <CostSortHeader field="net_margin_pct" label="Profit Margin %" sortConfig={costSort} onSort={handleCostSort} />
                          </>
                        )}
                    ...
                    <tbody className="font-semibold text-slate-700">
                      ...
                      {costSort ? (
                        sortedAnalysis.map((row) => (
                          <CostingLedgerRow key={row.sku} row={row} showCategory={true} showFinancials={userRole === "owner"} onViewBom={fetchDetails} />
                        ))
                        ...
                        return (
                          <React.Fragment key={categoryName}>
                            ...
                            <tr className="select-none border-t-2 border-slate-200 bg-[#885625]/5">
                              <th scope="rowgroup" colSpan={userRole === "owner" ? 8 : 2} className="px-4 py-3 text-left sm:px-5">
                              ...
                              return (
                                <React.Fragment key={sizeGroup.label}>
                                  ...
                                  <tr className="select-none border-y border-slate-100 bg-slate-50/70">
                                    <th scope="rowgroup" colSpan={userRole === "owner" ? 8 : 2} className="px-4 py-2.5 text-left text-xs font-black uppercase tracking-wider text-slate-500 sm:px-5">
                                      {sizeGroup.label}
                                    </th>
                                  </tr>

                                  {sizeGroup.items.map((row) => (
                                    <CostingLedgerRow key={row.sku} row={row} showCategory={false} showFinancials={userRole === "owner"} onViewBom={fetchDetails} />
                                  ))}
                                </React.Fragment>
                              );
                            })}
      ...
      {selectedSku && (
        ...
        >
          {detailsLoading ? (
            <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-slate-500 lg:min-h-[calc(85dvh-7rem)]">
              <RefreshCw className="animate-spin text-primary" size={32} />
              <span className="text-sm">Compiling costing breakdown...</span>
            </div>
          ) : details ? (
            isEditing ? (
              <div className="space-y-6 text-sm font-semibold leading-relaxed text-slate-700 lg:min-h-[calc(85dvh-7rem)]">
                {/* Product header info */}
                ...
                {userRole === "owner" && (() => {
                  const live = liveDraftCost;
                  if (!live) {
                    return (
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-bold text-amber-800">
                        Complete the draft recipe fields to calculate a normalized live cost preview.
                      </div>
                    );
                  }
                  const srp = selectedSellingPrice;
                  const draftTotalCost = live.portionCost + selectedLaborCost + selectedUtilityCost;
                  const netProfit = srp - draftTotalCost;
                  ...
                  return (
                    <div className="p-4 bg-[#885625]/5 border border-[#ece5da] rounded-2xl space-y-3">
                      <span className="text-xs text-primary font-black uppercase tracking-wider block flex items-center gap-1.5">
                        ...
                        {liveDraftCostLoading && <span className="ml-auto text-[10px] text-slate-400">Updating…</span>}
                      </span>
                      <div className="grid grid-cols-2 gap-4 text-xs font-bold text-slate-655 sm:grid-cols-3 xl:grid-cols-6">
                        <div>
                          <span className="text-slate-400 block text-[10px] uppercase font-black">Draft Batch Food</span>
                          <span className="text-base font-black text-slate-800 block font-mono mt-1">{formatCurrency(live.batchCost)}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[10px] uppercase font-black">Draft Food / Unit</span>
                          <span className="text-base font-black text-slate-800 block font-mono mt-1">{formatCurrency(live.portionCost)}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[10px] uppercase font-black">Labor / Unit</span>
                          <span className="text-base font-black text-slate-800 block font-mono mt-1">{formatCurrency(selectedLaborCost)}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[10px] uppercase font-black">Utility / Unit</span>
                          <span className="text-base font-black text-slate-800 block font-mono mt-1">{formatCurrency(selectedUtilityCost)}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block text-[10px] uppercase font-black">Draft Total / Unit</span>
                          <span className="text-base font-black text-slate-800 block font-mono mt-1">{formatCurrency(draftTotalCost)}</span>
                        </div>
                        <div>
                ...
                <div className="space-y-2">
                  ...
                  <DataTableScroll
                    ...
                    className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-3xs"
                  >
              ...
              {userRole === "owner" && (() => {
                const srp = selectedSellingPrice;
                const cost = selectedTotalCost;
                const netProfit = selectedNetProfit;
                const marginPct = selectedMarginPct;
                const foodPct = getCostPercentage(selectedFoodCost, srp);
                const laborPct = getCostPercentage(selectedLaborCost, srp);
                const utilityPct = getCostPercentage(selectedUtilityCost, srp);
                const hasFinancialReference = srp > 0;
                const isNegative = hasFinancialReference && netProfit < 0;
                const isLowMargin = hasFinancialReference && marginPct < 40;

                return (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 lg:grid-cols-4 xl:grid-cols-7">
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Batch Food Cost</span>
                        <span className="mt-1 block font-mono text-base font-black text-slate-800">{formatCurrency(details.calculated_batch_cost)}</span>
                        <span className="mt-1 block text-[10px] font-mono text-slate-455">Yield: {details.yield_weight}{details.yield_unit}</span>
                      </div>
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Food Cost / Unit</span>
                        <span className="mt-1 block font-mono text-base font-black text-slate-800">{formatCurrency(selectedFoodCost)}</span>
                        <span className="mt-1 block text-[10px] font-bold text-slate-455">
                          {foodPct === null ? "No SRP" : `${foodPct.toFixed(2)}% of SRP`}
                        </span>
                      </div>
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Labor Cost / Unit</span>
                        <span className="mt-1 block font-mono text-base font-black text-slate-800">{formatCurrency(selectedLaborCost)}</span>
                        <span className="mt-1 block text-[10px] font-bold text-slate-455">{laborPct === null ? "No SRP" : `${laborPct.toFixed(2)}% of SRP`}</span>
                      </div>
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Utility Cost / Unit</span>
                        <span className="mt-1 block font-mono text-base font-black text-slate-800">{formatCurrency(selectedUtilityCost)}</span>
                        <span className="mt-1 block text-[10px] font-bold text-slate-455">{utilityPct === null ? "No SRP" : `${utilityPct.toFixed(2)}% of SRP`}</span>
                      </div>
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Total Cost / Unit</span>
                        <span className="mt-1 block font-mono text-base font-black text-slate-800">{formatCurrency(cost)}</span>
                        <span className="mt-1 block text-[10px] font-bold text-slate-455">Food + labor + utility</span>
                      </div>
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Retail SRP</span>
                        <span className="mt-1 block font-mono text-base font-black text-slate-800">{hasFinancialReference ? formatCurrency(srp) : "Unavailable"}</span>
                        <span className="mt-1 block text-[10px] font-bold text-slate-455">Current product price</span>
                      </div>
                      <div>
                        <span className="block text-[10px] font-black uppercase text-slate-505">Profit Margin</span>
                        <span className={`mt-1 block font-mono text-base font-black ${marginPct < 40 ? "text-amber-600" : "text-emerald-600"}`}>
                          {hasFinancialReference ? `${marginPct.toFixed(2)}%` : "Unavailable"}
                        </span>
                        <span className="mt-1 block text-[10px] font-bold text-slate-455">{hasFinancialReference ? `${formatCurrency(netProfit)} net profit` : "Retail SRP required"}</span>
                      </div>
                    </div>
                        ...
                        <div>
                          ...
                          <span className="text-xs font-semibold leading-relaxed block mt-1">
                            {isNegative
                              ...
                              : `The profit margin is ${marginPct.toFixed(2)}% which is below our 40% goal. Consider adjusting the SRP to ${formatCurrency(cost * 1.6)}.`}
                          </span>
                        </div>
                      </div>
                    )}

                    {/* Cost composition explanation */}
                    <div className="p-4 bg-[#885625]/5 border border-[#ece5da] rounded-2xl space-y-2">
                      <span className="text-xs text-primary font-black uppercase tracking-wider block flex items-center gap-1.5">
                        <TrendingUp size={16} /> Cost Composition
                      </span>
                      <p className="text-xs text-slate-600 leading-normal font-medium">
                        These figures use the latest product costing analysis and current retail SRP. Food contributes <strong className="font-mono text-slate-800">{foodPct?.toFixed(2) ?? "0.00"}%</strong>, labor <strong className="font-mono text-slate-800">{laborPct?.toFixed(2) ?? "0.00"}%</strong>, and utility <strong className="font-mono text-slate-800">{utilityPct?.toFixed(2) ?? "0.00"}%</strong> of the selling price.
                      </p>
                    </div>
                  </div>
                );
            ...
            <div className="py-8 text-center text-slate-500 lg:min-h-[calc(85dvh-7rem)]">Error loading details.</div>
          )}
        </Modal>
      )}
