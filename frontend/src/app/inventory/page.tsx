...
export default function InventoryPage() {
  ...
  const handleSaveProductEdit = async () => {
    ...
    try {
      await api.updateProduct(editingProduct.sku, {
        ...
        labor_cost: !isNaN(parseFloat(editingProduct.labor_cost)) ? parseFloat(editingProduct.labor_cost) : 0.0,
        utility_cost: !isNaN(parseFloat(editingProduct.utility_cost)) ? parseFloat(editingProduct.utility_cost) : 3.28,
  ...
  return (
    <div className="space-y-6 flex flex-col">
      ...
      {/* Tabs Menu */}
      <div className="rounded-2xl border border-slate-200 bg-white/80 p-3 sm:hidden">
        <label htmlFor="mobile-inventory-view" className="mb-1.5 block text-[10px] font-black uppercase tracking-wider text-slate-500">Inventory view</label>
        <select
          id="mobile-inventory-view"
          value={activeTab}
          onChange={event => setActiveTab(event.target.value as typeof activeTab)}
          className="h-12 w-full rounded-xl border-2 border-slate-200 bg-white px-3 text-sm font-black text-slate-800"
        >
          {tabs.map(tab => <option key={tab.id} value={tab.id}>{tab.label}</option>)}
        </select>
      </div>
      <div className="hidden gap-1 whitespace-nowrap rounded-2xl border border-slate-200 bg-white/70 p-1.5 sm:flex" role="tablist" aria-label="Inventory views">
        {tabs.map((tab) => (
          <button
            key={tab.id}
