...
import { calculateConsignmentUnitPrice } from "@/lib/consignmentPricing";
...
export default function ConsignmentPage() {
  ...
  const handleDeleteDelivery = async (deliveryId: number) => {
    if (!window.confirm("Are you sure you want to permanently delete this delivery log? This will restore the dispatched quantities back to warehouse stock.")) {
      return;
    }
    try {
      await api.deleteConsignmentDelivery(deliveryId);
      if (selectedPartnerId !== null) {
        await refreshDeliveries(selectedPartnerId);
      }
      await fetchPartners();
      await fetchProducts();
      alert("Delivery log successfully deleted and stock restored.");
    } catch (err: unknown) {
      alert(`Error deleting delivery: ${getErrorMessage(err)}`);
    }
  };
  ...
  const activePartner = partners.find((partner) => partner.id === selectedPartnerId);
  const productsBySku = useMemo(
    () => new Map(products.map((product) => [product.sku, product])),
    [products],
  );
  const draftDispatchValue = deliveryItems.reduce((total, item) => {
    const product = productsBySku.get(item.sku);
    if (!product) return total;

    const unitPrice = calculateConsignmentUnitPrice(
      Number(product.retail_price),
      Number(activePartner?.discount_rate ?? 0),
    );
    return total + item.target_qty * unitPrice;
  }, 0);
                  ...
                  {deliveriesLoading ? (
                    ...
                    <div className="divide-y divide-slate-200">
                      {deliveries.map((delivery) => {
                        ...
                        const dispatchValue = delivery.items.reduce(
                          (total, item) => total + item.qty_delivered * item.reseller_price_snapshot,
                          0,
                        );
                        ...
                        return (
                          <div key={delivery.id} className="p-6 md:p-8 space-y-6">
                            ...
                            <div className="flex flex-col xl:flex-row xl:justify-between xl:items-center gap-4 bg-slate-50 p-5 border border-slate-200 rounded-2xl shadow-3xs">
                              ...
                              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-right shadow-3xs">
                                <span className="block text-[10px] font-black uppercase tracking-wider text-amber-800">Total sent</span>
                                <strong className="block font-mono text-lg font-black text-slate-900" title="Quantity dispatched multiplied by the delivery's snapshotted wholesale price">
                                  {formatCurrency(dispatchValue)}
                                </strong>
                                <span className="block text-[10px] font-semibold text-amber-800">Dispatch value</span>
                              </div>
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    {userRole === "owner" && (
                                      <Button
                                        ...
                                        className="h-10 w-10 p-0 border-rose-300 text-rose-700 hover:bg-rose-50 rounded-xl flex items-center justify-center cursor-pointer"
                                      >
                                        <Trash2 size={16} />
                                      </Button>
                                    )}
                            ...
                            <DataTableScroll label={`Shipment items for ${delivery.dr_number || "draft DR"}`} className="hidden md:block overflow-x-auto border border-slate-200 rounded-2xl">
                              <table className="w-full min-w-[1120px] text-left border-collapse text-sm text-slate-700">
                                <thead>
                                  <TableHeaderRow>
                                    <TableHeaderCell className="sticky left-0 bg-slate-100/90 backdrop-blur-xs z-10 shadow-3xs border-r border-slate-200/80">Finished SKU</TableHeaderCell>
                                ...
                                <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                                  {delivery.items.map((item) => {
                                    ...
                                    const matchedProduct = productsBySku.get(item.sku);
                                    const identity = {
                                      sku: item.sku,
                                    ...
                                    return (
                                      <TableRow key={item.id}>
                                        <TableCell className="sticky left-0 bg-white z-10 shadow-3xs border-r border-slate-200/80">
                                          <ProductDisplay
                                            sku={identity.sku}
        ...
        >
          <div className="space-y-6 text-sm font-semibold text-slate-600">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs text-slate-450 font-bold uppercase tracking-wider block mb-1.5">DR Number (optional)</label>
                <input
                  type="text"
                ...
                />
                <p className="mt-1.5 text-xs font-medium text-slate-500">You can replace the generated number later with the official paper DR.</p>
              </div>
              <div>
            ...
            <div className="flex items-center justify-between gap-4 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
              <div>
                <p className="text-xs font-black uppercase tracking-wider text-amber-800">Estimated dispatch value</p>
                <p className="mt-1 text-xs font-semibold text-slate-600">Based on selected quantities and {activePartner?.name ?? "the partner"}&apos;s current discount from SRP.</p>
              </div>
              <strong className="shrink-0 font-mono text-xl font-black text-slate-900">{formatCurrency(draftDispatchValue)}</strong>
            </div>
