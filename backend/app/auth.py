...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security), db: Session = Depends(get_db)) -> models.User:
    """Validate the token and return the current active database user."""
    ...
    # Access-token claims identify the account, but database state is authoritative
    # for activation and authorization. Tokens with an id must continue to match
    # both the original user id and username; legacy tokens fall back to username.
    if user_id is not None:
        ...
        if normalized_user_id is not None:
            user = db.query(models.User).filter(
                models.User.id == normalized_user_id,
