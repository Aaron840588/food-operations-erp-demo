...
def create_reseller_order(payload: schemas.ResellerOrderCreate, db: Session = Depends(get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    try:
        ...
        cost_snapshots = build_unit_cost_snapshots(
            db,
            [item["product"] for item in items_to_create],
        )
        ...
        tax_amt = discounted_subtotal * (tax_rate / (100.0 + tax_rate))
        ...
        for item in items_to_create:
            cost_snapshot = cost_snapshots[item["sku"]]
...
@router.delete("/orders/{order_id}", dependencies=[Depends(auth.require_owner)])
def delete_reseller_order(
    order_id: int, 
    db: Session = Depends(get_db), 
    current_user: models.User = Depends(auth.require_owner)
):
    """
    Deletes a reseller order.
    Restores the quantities of all items in the order back to warehouse stock.
    Logs movement transactions.
    """
    order = db.query(models.ResellerOrder).filter(models.ResellerOrder.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Reseller order not found")
    ...
    try:
        ...
        db.query(models.ProductionBatch).filter(
            models.ProductionBatch.notes.like(f"%Invoice #{order.id}%")
        ).delete(synchronize_session=False)

        # Keep warehouse stock sync
        from ..database import sync_warehouse_stock_for_main_facility
        skus_to_sync = [item.sku for item in order.items]
    ...
    return {"message": f"Successfully deleted reseller order #{order_id} and restored warehouse stock."}
...
@router.post("/discount-tiers", response_model=schemas.DiscountTierOut, dependencies=[Depends(auth.require_owner)])
...
@router.put("/discount-tiers/{tier_id}", response_model=schemas.DiscountTierOut, dependencies=[Depends(auth.require_owner)])
...
@router.delete("/discount-tiers/{tier_id}", dependencies=[Depends(auth.require_owner)])
