...
@router.put("/overhead-rates/{category}", response_model=schemas.CategoryOverheadRateOut)
...
def create_gift_set(payload: schemas.GiftSetCreate, db: Session = Depends(get_db)):
    ...
    calculated = calculate_gift_set_costs(db, db_gift)

    # Sync corresponding ProductSKU so it appears under GIFT SETS & PACKAGES category
    sku_code = f"GS-{db_gift.id}"
    prod_sku = db.query(models.ProductSKU).filter(models.ProductSKU.sku == sku_code).first()
    if not prod_sku:
        prod_sku = models.ProductSKU(
            sku=sku_code,
            product_name=db_gift.name,
            category="giftset",
            size="Bundle",
            retail_price=db_gift.retail_price,
            reseller_price=db_gift.reseller_price,
            cost_per_unit=calculated.calculated_total_cost,
            warehouse_stock=10,
            is_active=True
        )
        db.add(prod_sku)
    else:
        prod_sku.product_name = db_gift.name
        prod_sku.retail_price = db_gift.retail_price
        prod_sku.reseller_price = db_gift.reseller_price
        prod_sku.cost_per_unit = calculated.calculated_total_cost
        prod_sku.is_active = True
