from sqlalchemy import Column, Integer, String, Float, ForeignKey, Boolean, Date, Text, DateTime, func, CheckConstraint
...
class RecipeItem(Base):
    ...
    raw_ingredient_id = Column(Integer, ForeignKey("raw_ingredients.id", ondelete="SET NULL"))
    sub_sku = Column(String(100), ForeignKey("product_skus.sku", ondelete="SET NULL"))
...
class ProductionBatch(Base):
    ...
    sku = Column(String(100), ForeignKey("product_skus.sku", ondelete="SET NULL"))
...
class GiftSetItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    gift_set_id = Column(Integer, ForeignKey("gift_sets.id", ondelete="CASCADE"), nullable=False)
    sku = Column(String(100), ForeignKey("product_skus.sku", ondelete="CASCADE"), nullable=False)
    quantity = Column(Integer, nullable=False)
...
class WarehouseStock(Base):
    ...
    raw_ingredient_id = Column(Integer, ForeignKey("raw_ingredients.id", ondelete="CASCADE"), nullable=True)
    sku = Column(String(100), ForeignKey("product_skus.sku", ondelete="CASCADE"), nullable=True)
