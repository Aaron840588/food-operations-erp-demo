...
class ConsignmentTests(unittest.TestCase):
    ...
    def test_record_consignment_delivery_generates_dr_number_when_blank(self):
        self.add_product("SKU-1", 15)
        payload = schemas.ConsignmentDeliveryCreate(
            partner_id=1,
            delivery_date="2026-07-16",
            dr_number="   ",
        ...
        self.assertRegex(delivery.dr_number or "", r"^DR-20260716-\d{5}$")

    def test_record_consignment_delivery_snapshots_partner_discount_price(self):
        product = self.add_product("SKU-1", 15)
        product.retail_price = 245
        product.reseller_price = 210
        self.db.commit()
        payload = schemas.ConsignmentDeliveryCreate(
            partner_id=1,
            delivery_date="2026-08-03",
            dr_number="DR-OTOP-PRICE",
            items=[schemas.ProductionTargetBase(sku="SKU-1", target_qty=2, outlet="Consignment")],
        )

        delivery = record_consignment_delivery(payload, self.db, self.user)

        self.assertEqual(delivery.items[0].reseller_price_snapshot, 221)
        self.assertEqual(delivery.items[0].store_price_snapshot, 245)

    def test_delete_consignment_delivery_restores_stock(self):
        from app.routers.consignment import delete_consignment_delivery
        product = self.add_product("SKU-1", 15)
        payload = schemas.ConsignmentDeliveryCreate(
            partner_id=1,
            delivery_date="2026-07-16",
        ...
        response = delete_consignment_delivery(delivery.id, self.db, self.user)
        self.db.refresh(product)
        # Stock should be restored to 15!
        self.assertEqual(product.warehouse_stock, 15)
        self.assertEqual(self.db.query(models.ConsignmentDelivery).count(), 0)
        self.assertEqual(self.db.query(models.ConsignmentItem).count(), 0)
