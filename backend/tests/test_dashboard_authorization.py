...
class DashboardAuthorizationTests(unittest.TestCase):
    ...
    def test_staff_summary_omits_finance_while_owner_retains_it(self):
        ...
        self.assertIn("owner_weekly", owner_payload)
    ...
    def test_dashboard_handles_numeric_market_revenue_with_float_aggregates(self):
        db = self.session_factory()
        product = models.ProductSKU(
            sku="DASH-MARKET",
            product_name="Dashboard Market Product",
            category="Savory",
            size="100g",
            retail_price=100.0,
            reseller_price=80.0,
        ...
        item = models.MarketEventSaleItem(
            sale_id=1,
            sku=product.sku,
            quantity=1,
            price_snapshot=100,
        )
        db.add_all([product, event, sale, item])
        db.commit()
        db.close()

        summary_response = self.client.get("/dashboard/summary")
        analytics_response = self.client.get("/dashboard/analytics")

        self.assertEqual(summary_response.status_code, 200)
        self.assertEqual(analytics_response.status_code, 200)
        self.assertEqual(summary_response.json()["analytics"]["market_sales"], 100.0)
        self.assertEqual(summary_response.json()["analytics"]["combined_sales"], 100.0)
        self.assertEqual(analytics_response.json()["market_sales"], 100.0)
        self.assertEqual(analytics_response.json()["combined_sales"], 100.0)
